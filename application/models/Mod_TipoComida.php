...
class Mod_TipoComida extends CI_Model {
    ...
    {
        $response = array('message' => 'No se pudo eliminar el tipo de comida', 'done' => false);
        ...
        if ($tipos){
            if ($tipos->totalPlatillos == 0){
                ...
                $response['message'] .= ' debido a que está siendo uti lizado por un platillo';
        }else
    ...
    // var_dump($values);
        ...
        {
            $response['message'] = 'Ya existe un tipo de comida con lo s datos que indicó';
            return $response;
        }
