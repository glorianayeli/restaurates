...
class TipoComida extends MY_Controller {
    ...
    public function Listado()
    {
        ...
        if($params['registros']){
            $config = $this->ConfigurarPaginacion( base_url().'TipoComida/Listado',$params['totalRegistros']
        );
    ...
    {
        $this->session->set_flashdata('response', $this->mod_tipocomida->Eliminar($id));
    ...
    public function Guardar()
    {
        ...
        $this->session->set_flashdata('response', $response); redirect('TipoComida/Listado','refresh');
    }
}
